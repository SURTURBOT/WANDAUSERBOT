...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    THANOSBOTevent = await edit_or_reply(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await THANOSBOTevent.edit("`Please unblock `@DrWebBot `and try again`")
            return
        if response.text.startswith("Forward"):
            await THANOSBOTDevent.edit(
...
async def _(event):
    if event.fwd_from:
        return
    THANOSBOTevent = await edit_or_reply(event, "...")
    start = datetime.now()
    ...
    try:
        ...
        await THANOSBOTevent.edit(str(e))
        return
    end = datetime.now()
    ms = (end - start).seconds
    await THANOSBOTevent.edit("Created BarCode in {} seconds🤓".format(ms))
...
async def _(event):
    if event.fwd_from:
        return
    THANOSBOTevent = await edit_or_reply(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await THANOSBOTevent.edit(output_str, link_preview=True)
    else:
        await THANOSBOTevent.edit("xkcd n.{} not found!".format(xkcd_id))
