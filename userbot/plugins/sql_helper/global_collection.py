...
class THANOSBOT_GlobalCollection(BASE):
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, THANOSBOT_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


THANOSBOT_GlobalCollection.__table__.create(checkfirst=True)

THANOSBOT_GLOBALCOLLECTION = threading.RLock()
...
def rm_from_collectionlist(keywoard, contents):
    with THANOSBOT_GLOBALCOLLECTION:
        keyword_items = SESSION.query(THANOSBOT_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with THANOSBOT_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(THANOSBOT_GlobalCollection.keywoard)
            .filter(THANOSBOT_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(THANOSBOT_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(THANOSBOT_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(THANOSBOT_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(THANOSBOT_GlobalCollection).all()
        for x in all_groups:
