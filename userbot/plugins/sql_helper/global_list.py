...
class THANOSBOTGloballist(BASE):
    __tablename__ = "THANOSBOTglobal_list"
    ...
    def __repr__(self):
        return "<THANOSBOT global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, THANOSBOTGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


THANOSBOTGloballist.__table__.create(checkfirst=True)

THANOSBOTGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with THANOSBOTGLOBALLIST_INSERTION_LOCK:
        broadcast_group = THANOSBOTGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with THANOSBOTGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(THANOSBOTGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with THANOSBOTGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(THANOSBOTGloballist.keywoard)
            .filter(THANOSBOTGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(THANOSBOTGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(THANOSBOTGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(THANOSBOTGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(THANOSBOTGloballist).all()
        for x in all_groups:
