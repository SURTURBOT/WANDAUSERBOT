#this plugin maked by girls plugin maker @thanosceo
import os
import random
import time
...
@bot.on(admin_cmd(pattern="glogo (.*)"))
@bot.on(sudo_cmd(pattern="glogo (.*)", allow_sudo=True))
async def lg1(THANOSBOT):
    ...
    if THANOSBOT.reply_to_msg_id:
        ...
        async for i in bot.iter_messages("@THANOSGIRLSDP", filter=InputMessagesFilterPhotos):
    	    PICS_STR.append(i)
...
CmdHelp("GIRLSlogo").add_command(
  "glogo", "<reply to pic + text> or <text>", "Makes a girllogo with the given text. If replied to a picture makes logo on that else gets random girls BG."
).add_info(
