...
async def info(event):
    ...
    if input_str == "text":
        ...
        for i in sorted(SUDO_LIST):
            ...
            string += "\n"
        if len(string) > 4095:
            data = string.format(count=THANOSBOTcount, plugincount=plugincount)
            key = (
                requests.post(
            ...
            reply_text = f"All commands of the THANOSBOT are [here]({url})"
            await event.reply(reply_text, link_preview=False)
            return
        await event.reply(
            string.format(count=THANOSBOTcount, plugincount=plugincount), link_preview=False
        )
        return
    if input_str:
        if input_str in SUDO_LIST:
            ...
            for i in SUDO_LIST[input_str]:
                ...
                THANOSBOTcount += 1
            await event.reply(
                string.format(count=THANOSBOTcount, input_str=input_str), parse_mode="HTML"
            )
        else:
        ...
        for i in sorted(SUDO_LIST):
            ...
            string += " "
            THANOSBOTcount += 1
        await event.reply(string.format(count=THANOSBOTcount), parse_mode="HTML")
