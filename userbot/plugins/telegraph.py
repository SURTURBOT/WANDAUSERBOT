...
@bot.on_cmd(
    ...
    info={
        ...
        "description": "Reply to text message to paste that text on telegraph you can also pass input along with command             So that to customize title of that telegraph and reply to media file to get sharable link of that media(atmost 5mb is supported)",
        "options": {
...
async def _(event):
    ...
    THANOSBOTevent = await eor(event, "`processing........`")
    if BOTLOG:
...
**Do not give this url to anyone, even if they say they are from Telegram!**",
    ...
    if input_str in ["media", "m"]:
        ...
        await THANOSBOTevent.edit(f"`Downloaded to {downloaded_file_name}`")
        if downloaded_file_name.endswith((".webp")):
            resize_image(downloaded_file_name)
        try:
            media_urls = upload_file(downloaded_file_name)
        except exceptions.TelegraphException as exc:
            await THANOSBOTevent.edit(f"**Error : **
`{exc}`")
...
", "<br>")
        ...
        ms = (end - start).seconds
        THANOSBOT = f"https://telegra.ph/{response['path']}"
        await THANOSBOTevent.edit(
            f"**✓ Uploaded to :-** [telegraph]({THANOSBOT})                 
**✓ Uploaded in {ms} seconds.**                 
**✓ Uploaded by :-** {mention}                 
**✓ Telegraph :-** `{THANOSBOT}`",
            link_preview=True,
