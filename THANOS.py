...
lnbot = TelegramClient("thanosuser", API_ID, API_HASH).start(bot_token=token)


class thanosuser(TelegramClient):
...
if Config.THANOS_STRING:
    session_name = str(Config.THANOS_STRING)
    sweetie = thanosuser(session_name, Config.APP_ID, Config.API_HASH)
else:
